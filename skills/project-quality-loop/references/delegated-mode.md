...
Every RUN dispatches the verifier on the session model - a seat carries no model pin of its own. Running only the thorough first RUN of a stage on a stronger model, re-verify RUNs staying on the session model, is an OPT-IN experiment, not the default: it may catch materially more real issues, but that is an unproven change, so it stays OFF until a benchmark on a real target shows it pays. Do not silently ship it on.
...
- **Where and how.** Write it at the project root's `<docs-path>/flow/APPROVAL`. A write relative to the
  shell's working directory follows wherever that directory drifted and lands the stamp in a phantom nested
  docs tree the hook never reads.
...
- **Ownership.** Written when this session's own decision lands, deleted at its own close (the run's end,
  ...
  either.
- **When the write is refused.** If the stamp cannot be written, stop and put the choice to the user as one
  explicit question (retry, or run this stage inline). Never retry blind, and never let the run degrade to
  inline with nothing said - a stage that ran inline because the stamp could not be written is reported as
  exactly that.
...
is the SKILL.md bodies the seat reads before it starts - measured at 26k-70k chars (roughly 7k-17k
tokens) for the designer, implementer and verifier seats and zero for the support seats - and a
re-verify dispatch re-pays it in full, which is one more reason to batch a punch-list into one re-dispatch per seat. So: fold slices
