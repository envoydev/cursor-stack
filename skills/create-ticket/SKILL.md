...
- **Filing**: if an issue-tracker MCP is connected (e.g. Atlassian), offer it after presenting the ticket as one explicit question - file it via the connector vs copy-paste only (plain-text options where the harness lacks the tool); title and description map 1:1. Never file without explicit confirmation.
