...
- **Out of this router's scope.** Web front-end and mobile work belong to the Angular, TypeScript and Ionic/Capacitor conventions skills, whose descriptions lead with their own triggers - not here. Cross-cutting flow that is not .NET-specific - `/review`, `project-verify-code`, context7 library docs, git - lives in the project's `AGENTS.md`.
