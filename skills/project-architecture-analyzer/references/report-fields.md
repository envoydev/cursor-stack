...
several items - the close is an answer like any other, and a wall of prose buries the receipts, so they must
survive a short close rather than be re-answered away.
...
| `Write passes:` | per doc, what step 5's precondition allows: `ARCHITECTURE.md 1, ASSESSMENT.md 1` on a clean run, `2` where the budget spill fired, `1 (overlay, N sections)` on the overlay route where the doc is written section by section through `docs.js set`, and the honest number plus one line on what was still being verified where a third pass happened | the verification-first precondition |
...
there, and never an assertion that a reset happened. You cannot change it yourself: the session model is the
user's to switch, so the line ends in the exact model to switch back to. Three shapes, pick the true one:

- `raised for this run - switch the session model back to <the model this session was on before>`
- `already on the strongest model before this run started - nothing to reset` - allowed only when you can point
  at the message that proves it, since a new-session default says nothing about a session already running
- `not on the strongest model - the judgment above ran on <model>`
