...
## Conventions
- Follow the `ionic` and `angular-styling` skills - judge the diff against them directly, not recall. The house TypeScript, Angular, and styling conventions auto-attach via the `.cursor/rules/typescript-conventions.mdc`, `.cursor/rules/angular-conventions.mdc`, and `.cursor/rules/scss-conventions.mdc` rules.
...
3. Audit TypeScript code quality against the trap families below and the Angular checks, and the assembled `.scss` against `angular-styling`:
