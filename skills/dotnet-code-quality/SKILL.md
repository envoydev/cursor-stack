...
If the fix is genuinely too large, put it to the user as ONE explicit question naming the warning ID and the count, with three options: fix it now (recommended), defer that one ID with a `NoWarn` carrying a dated comment, or drop the promotion for this wave. Never take any of the three unilaterally - a silently weakened policy reads as a green build. (See the reward-hacking shortcuts list below; enforce it up front rather than catch it after the fact.)
