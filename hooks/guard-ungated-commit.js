...
// Cursor `beforeShellExecution` hook: the PUBLISH ceremony, mechanized - one hook because commit
// and push are one gate family and share the receipt machinery, the heredoc blanking and the
// quote masking below. A non-trivial `git commit` runs only after the house review gate
// (project-verify-code, plus /review on auth/crypto/data-access paths) or the user's explicit
// waiver - recorded as a receipt file the gate step writes. Prose measured unreliable on the peer
// stack: 8 ungated commit events across 6 audited sessions, including one where the git baseline
// was provably in context the same session and skipped anyway, and one commit with no user
// authorization at all. Trivial diffs pass untouched (the rule's own typo/one-line exemption,
// judged from the working-tree diff).
//
// The PUBLISH half: `git push` and a PR merge put the work where other people (and CI) get it, and
// NOTHING gated them - replayed across four bundles on the peer stack, every push and merge passed
// every guard. In one session the FIRST state-changing act of the run published unpushed commits 18
// minutes before any receipt existed, and 40 files reached a shared `develop` ungated. Same receipt
// shape, its own file (<docs-root>/flow/PUSH-GATE), and CURSOR_PUSH_GATE=0 turns it off for a repo
// whose remote is already gated by branch protection or a required review.
//
// Receipt lifecycle: the gate step writes <docs-root>/flow/COMMIT-GATE when its checks pass
// (VERIFIED <scope>) or the user explicitly waives (WAIVED - "<their words>"); the commit turn
// clears it after the commit lands. Receipts older than MAX_RECEIPT_AGE_MS are treated as absent -
// the stale-stamp lesson from the approval gate (a leftover stamp silently authorized later,
// unrelated runs).
//
...
// The docs root env value - where the gate receipts and the block ledger live.
const docsRootEnv = () => process.env.CURSOR_DOCS_PATH || '.cursor/docs';

// --- Cursor hooks.json v1 contract ------------------------------------------------------------
// beforeShellExecution answers a permission object on stdout: there is no exit-2 denial and no
// stderr channel back to the model. The judgement below is the peer stack's, VERBATIM - a gate
// that decides differently on the two stacks is a gate nobody can reason about - so instead of
// rewriting 400 lines of it, this shim translates the boundary: stderr is captured, exit(0)
// becomes an allow and exit(2) becomes a deny carrying the captured text as the agent message.
let payload;
try {
  payload = JSON.parse(fs.readFileSync(0, 'utf8'));
} catch {
  process.stdout.write(JSON.stringify({ permission: 'allow' })); // unparseable stdin - don't block
  process.exit(0);
}
if (!payload || typeof payload !== 'object') {
  process.stdout.write(JSON.stringify({ permission: 'allow' })); // a JSON scalar/null - nothing to judge
  process.exit(0);
}
  ...
  process.exit = (code) => {
    if (code === 2) {
      ...
      process.stdout.write(JSON.stringify({
        permission: 'deny',
        user_message: last.split('\n')[0].slice(0, 300),
        agent_message: last.trim(),
      }));
    } else {
...
const PUSH_GATE_ON = process.env.CURSOR_PUSH_GATE !== '0';
    ...
    if (!((r.waived || r.verified) && !r.problem)) {
      process.stderr.write(
        (r.stale
          ...
          `this half off for good: CURSOR_PUSH_GATE=0 in the environment.`,
      );
      process.exit(2);
    }
  }
