...
- Never silently change a SHARED contract seam - a message shape, a queue/topic name, an exit code, a config key, a hosted-service registration order, a gateway event contract, or other cross-cutting behavior. A local detail you may change and report; a shared-seam change stops with a clear note of what needs to change and why, rather than altering the seam yourself.
...
Build it clean the first time - Generic-Host / long-running traps to hunt as you write (the skills carry the fix):
