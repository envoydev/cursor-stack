...
- Navigate with serena/LSP, not whole-file reads (the `.cursor/rules/baseline-navigation.mdc` baseline). Use `dotnet test --filter` to iterate on the failing test(s); run the full suite to confirm at the end.
