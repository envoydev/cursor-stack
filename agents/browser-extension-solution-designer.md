...
## Conventions
- **Load your skills first.** Cursor has no frontmatter preload, so the conventions only arrive if you fetch them: READ `.cursor/skills/browser-extension/SKILL.md`, `.cursor/skills/ts-js-testing/SKILL.md`, `.cursor/skills/project-solution-design/SKILL.md` before you start and follow them. Conventions are the source of truth, not recall - a prose reminder alone is the measured failure mode.
...
- `browser-extension`, `ts-js-testing`, and `project-solution-design` are read at start (above) - judge topology and fit against them directly. The language layer defers to `typescript` over its `javascript` base - every task contract you author holds the typescript baseline (no `any`, type-modeled message payloads), with those skills referenced on demand.
...
3. Set the plan and the test strategy - business logic in chrome-free TS modules unit-tested on the workspace's detected runner (per the `ts-js-testing` hub) with a fake chrome API (`@webext-core/fake-browser`, or the workspace's existing mock), and Playwright persistent-context E2E reserved for the flows only a real browser proves (injection, service-worker wake, the store-facing UI).
