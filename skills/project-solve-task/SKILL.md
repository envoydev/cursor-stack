...
A stop IS one explicit question to the user: report THREE named fields, then put the next move
through that question - EVERY stop, the plain step-done ones included.
...
recommended option; the pre-commit checkpoint and its receipt still apply. Write the stamp at the project root's `<docs-path>`, never relative to the shell's drifted working directory. The stamp belongs to the session that dispatches - written when its own decision lands, deleted at its own close; an earlier session's leftover stamp is not consent. `references/step-mechanics.md` carries the rest of the mechanics - why a relative write bounces the dispatch, what to do when the stamp cannot be written, and the AUTO stamp's lifetime across steps 4-6.

## The steps

Each step that names a skill INVOKES it - loads its SKILL.md fresh - and re-invokes it for every new
...
3. **APPROVE** - Read `references/step-mechanics.md` now - the mode-fit rule for this ask, the
   build bar, the step-4 reviewer-fit rule and step 6's doc-drift surfaces are its content, not homework; this
   stop's `Result:` line carries `mechanics: read` as the receipt. Then present the gated plan and
   put the gate through the stop contract's decision mechanism as ONE question whose options each
   NAME the mode: 'Approve - build in this session', 'Approve - dispatch the agent seats', 'Not
   yet - changes needed'. Mark recommended the mode that fits THIS plan per the mode-fit rule, with
   the reason in the option's description - a fixed default is not a recommendation. Approval and mode arrive as one answer by construction - the bare 'go'
...
4. **BUILD** - per the approved mode:
   ...
   - *agents*: fan the plan's task cards out to the matching `<stack>-implementer` seats - flat
     ...
     session the only orchestrator; a red build/test routes per the repair-agent rules; tick the same plan file
     ...
     seat's green gate stays fast - build + fast tests, never integration replays or another
     minutes-long run; the slow full run happens once, in this session, at the step-5 review /
     step-6 done-gate.
   ...
   *Stop* - and this stop chooses the reviewer for step 5, through the same decision mechanism:
   ...
   recommended what fits the assembled diff per the reviewer-fit rule, reason stated; skip is
   never the recommendation.
5. **CONFORMANCE** (unless skipped - a skip is stamped `Conformance: skipped by user`, an honest
   record, not a silent gap) - INVOKE the reviewer chosen at the step-4 stop: in-session means loading
   and running `project-verify-code`, the seat means a Task dispatch - recording the
   choice and reviewing from memory of an earlier load is not running it, and a COMMIT-GATE
   receipt may only name a review that actually ran.
...
6. **CLOSE** - apply any fixes the step-5 review handed back, then the done-gate
   ...
   sibling's actual source before writing what it must do.
...
- Never pass a stop without the user's explicit word, and never approve the plan yourself - the
  ...
  approves nothing.
- Never end a stop's turn without its explicit question (or the plain-text fallback's option
