...
Cursor pins no model or effort per seat - every seat inherits the session model - so there is no in-stack lever that puts a stronger model on design and a cheaper one on routing. The choice is the session model you launch the flow on, and it applies to the whole run: pick the stronger model for a run whose value is in the DESIGN (a cross-domain feature, an unfamiliar area), the cheaper one for a run that is mostly routing and review over an already-frozen contract. Where the design matters and the budget does not stretch, split the work across two sessions rather than averaging the model down across one.
