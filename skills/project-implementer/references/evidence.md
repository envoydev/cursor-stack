...
- **The plan needs an `Approved:` line (Build mode 6).** A build started on a bare 'implement' with
  no approval line and no question, and the user rejected it after the whole run had been spent.
- **The red-gate loop stops at five (protocol step 3).** Seven red runs of one task's gate, each
  re-sending a large carried context for 47k chars of test output in total - the last two on code
...
- **A `FAILED` task ends the turn with a question (protocol step 4).** Forty consecutive tool calls,
  the plan saying the failure was reported, no question ever asked.
