...
## Conventions
- **Load your skills first.** Cursor has no frontmatter preload, so the conventions only arrive if you fetch them: READ `.cursor/skills/csharp-design-patterns/SKILL.md`, `.cursor/skills/dotnet/SKILL.md`, `.cursor/skills/dotnet-hosted-services/SKILL.md`, `.cursor/skills/dotnet-testing/SKILL.md`, `.cursor/skills/project-solution-design/SKILL.md` before you start and follow them. Conventions are the source of truth, not recall - a prose reminder alone is the measured failure mode.
