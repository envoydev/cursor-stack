...
```
plan .cursor/docs/superpowers/plans/csv-export.md:
...
A stop IS one an explicit question call: report one line of result and the artifact path, then put
the next move through an explicit question to the user - EVERY stop, the plain step-done ones included.
...
emitting any stop's an explicit question, ask 'has this cycle crossed the trigger?' - if yes and the
...
4. **BUILD** - per the approved mode:
   ...
   *Stop* - and this stop chooses the reviewer for step 5, through the same decision mechanism:
   ...
   invoke `/review` themselves - it is not part of this flow.) The user can inspect the diff
...
- Never pass a stop without the user's explicit word, and never approve the plan yourself - the
  ...
  approves nothing.
- Never end a stop's turn without its an explicit question (or the plain-text fallback's option
