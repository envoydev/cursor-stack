...
| `.cursor/rules/baseline-project-architecture.mdc` (GENERATED - user-run /project-architecture-analyzer) | architecture docs pointer - where the docs live and how to read them by section; the orientation itself arrives through the docs hook's session-start block (not a dispatched subagent) |
...
| `.cursor/rules/project-code-style.mdc` (GENERATED - user-run /project-code-style-analyzer; glob-scoped, plus the full doc) | the project's actual code style - the condensed core auto-attaches on any matching file touch (main session and subagents); the full capture stays in `<docs-path>/code-style/CODE-STYLE.md` |
