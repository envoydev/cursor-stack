...
(frontmatter, `Captured:` line, the stamped usage policy, the three inventory headings) is the copy
...
## MCP routing
One row per REGISTERED server only, from the routing map below - a server absent from
`.cursor/mcp.json` (and from the session's own MCP tool list) gets no row, and an unknown server
gets its name + 'routing: see project docs'. A row says WHEN to reach for the server and which of
...
## Plugins
No section. Cursor exposes no scriptable plugin inventory, so the generated rule carries no plugins
